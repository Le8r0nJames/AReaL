...
def _install_aweagent_deps_on_ray_nodes(aweagent_root: str):
    """Install AWEAgent dependencies on all Ray GPU nodes.
    ...
    if not aweagent_root:
        aweagent_root = str(Path(__file__).resolve().parents[2].parent / "AWEAgent")
    try:
        import ray
        ...
        for ip, rc, err in results:
            if rc != 0:
                logger.warning(f"Failed to install AWEAgent deps on {ip}: {err}")
            else:
                logger.info(f"AWEAgent deps installed on {ip}")
    except Exception as e:
        logger.warning(f"Could not install AWEAgent deps on Ray nodes: {e}")
