"""AWEAgent workflow for AReaL proxy mode.

This module runs AWEAgent-backed SWE-bench agents through AReaL's proxy
server during RL training. AWEAgent stays in a separate checkout and is
...
logger = logging.getLogger("AWEAgent")
...
def _default_aweagent_root() -> str:
    """Return the default sibling checkout path for AWEAgent."""
    areal_root = os.path.dirname(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    return os.path.join(os.path.dirname(areal_root), "AWEAgent")


def _ensure_aweagent_importable(aweagent_root: str = "") -> str:
    """Add the AWEAgent checkout root to ``sys.path``.
    ...
    3. ``../AWEAgent`` relative to the AReaL repository root.

    Returns the resolved AWEAgent checkout root.
    """
    if aweagent_root:
    ...
    if os.path.isdir(root) and root not in sys.path:
        ...
        logger.info(f"Added AWEAgent root to sys.path: {root}")
    elif not os.path.isdir(root):
        logger.warning(
            f"AWEAgent root does not exist: {root}. "
...
class SWEAgentWorkflow:
    ...
    This workflow runs an AWEAgent agent type (``swe``, ``cc``, ``oh``,
    ...
    The workflow delegates to AWEAgent's ``run_agent_with_reward``, passing
    ...
    def __init__(
        ...
        # Ensure AWEAgent is importable at construction time so we get an early
    ...
    async def _run_episode(
        ...
        """Execute one episode through AWEAgent and return the reward.
        ...
        """
        # Imported lazily - AWEAgent is only added to sys.path at run time.
...
# Keep the historical import path stable while making the generic intent clear
# for new configs.
AWEAgentWorkflow = SWEAgentWorkflow
