...
These cover the ``group_boundaries`` argument added to ``Normalization`` so that
...
def test_variable_group_boundaries_normalize_each_group_independently():
    ...
    out = norm(x, group_boundaries=boundaries)
...
def test_group_boundaries_none_matches_fixed_group_size():
    """Without group_boundaries, behavior is unchanged (backward compatible)."""
    norm = Normalization(_group_norm_config())  # group_size=2
    x = torch.tensor([0.0, 2.0, 10.0, 14.0], dtype=torch.float32)

    out_default = norm(x)
    out_explicit = norm(x, group_boundaries=[2, 2])
...
def test_variable_group_with_singleton_group():
    """A size-1 group is handled via the group_sz==1 special cases."""
    cfg = NormConfig(
        mean_level="group",
        std_level="group",
        group_size=1,
    ...
    out = norm(x, group_boundaries=[1, 2])

    assert torch.isfinite(out).all()
    # group_size==1 + mean_leave1out -> mean 0, std 1 -> value passes through.
    assert math.isclose(out[0].item(), 7.0 / (1.0 + 1e-5), rel_tol=1e-4)


def test_group_boundaries_sum_mismatch_raises():
    ...
    with pytest.raises(ValueError, match="must equal"):
        norm(x, group_boundaries=[3, 2])  # sums to 5 != 4


def test_group_boundaries_non_positive_raises():
    """Boundaries containing a non-positive size are rejected."""
    norm = Normalization(_group_norm_config())
    x = torch.tensor([0.0, 1.0, 2.0, 3.0], dtype=torch.float32)  # bs=4
    with pytest.raises(ValueError, match="positive"):
        norm(x, group_boundaries=[4, 0])
...
def test_adv_norm_style_2d_variable_groups_normalize_per_group():
    ...
    out = adv_norm(x, loss_mask, group_boundaries=[3, 2])
